...
class SeedCreate extends AbstractCommand
{
     ...
     * @var string
     */
    protected static $defaultName = 'seeds:create';
    ...
    /**
     * Get the question that allows the user to select which seeds path to use.
     *
     * @param string[] $paths
     *
    ...
    /**
     * Returns the seeds path to create the seeder in.
     *
    ...
    protected function getSeedPath(InputInterface $input, OutputInterface $output)
    {
        ...
        if (empty($paths)) {
            throw new Exception('No seeds paths set in your Phinx configuration file.');
        }
        ...
        if (empty($paths)) {
            throw new Exception(
                'You probably used curly braces to define seeds path in your Phinx configuration file, ' .
                'but no directories have been matched using this pattern. ' .
                'You need to create a seeds directory manually.'
            );
        }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        // get the seeds path from the config
        ...
        if (!Util::isValidPhinxClassName($className)) {
            throw new InvalidArgumentException(sprintf(
                'The seeds class name "%s" is invalid. Please use CamelCase format',
                $className
            ));
        }
        ...
        $output->writeln('<info>using seeds base class</info> ' . $classes['$useClassName']);
