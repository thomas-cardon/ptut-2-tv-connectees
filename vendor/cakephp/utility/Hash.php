...
class Hash
{
     ...
     *
     * Given a set of UserController array data, from a `$UserController->find('all')` call:
     *
     * - `1.UserController.name` Get the name of the user at index 1.
     * - `{n}.UserController.name` Get the name of every user in the set of users.
     * - `{n}.UserController[id].name` Get the name of every user with an id key.
     * - `{n}.UserController[id>=2].name` Get the name of every user with an id key greater than or equal to 2.
     * - `{n}.UserController[username=/^paul/]` Get UserController elements with username matching `^paul`.
     * - `{n}.UserController[id=1].name` Get the Users name with id matching `1`.
     *
     ...
     *
     * ```
     * $result = Hash::format($users, ['{n}.UserController.id', '{n}.UserController.name'], '%s : %s');
     * ```
     *
